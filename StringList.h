...
class StringList // Treat this class as an Originator
{
public:
...
private:
	...
	//Doubles the capacity of the list if n == capacity
	void checkCapacity();

	// Helper function to deep copy the contents of another list
	void copyList(const StringList& lst);


	// Memento class keeps track of the current state of string list
	class Memento 
	{
	public:
		// Constructor
		Memento(string* stringList, int size, int capacity);
	...
	private:
		string* stringListState;
		int currentSize;
		int currentCapacity;
	};

	// History is the previous state to keep track of the mementos
	class History
	{
	public:
		// Default constructor
		...
		//push
		void pushHistory(Memento* newMemento);

	private:
		int top;
		int stackCapacity;
		Memento** mementos; // a collection (array) of mementos

		// Helper function to check if history (stack) is full
	...
	//  create a new memento when the array is updated
	// return a new memento which is a deep copy of the current state
	Memento* createMemento();

	// Method to retrieve earlier memento (undo)
	// Assume the arr is not nullptr
	void undoHelper(Memento* paramMemento);
};
